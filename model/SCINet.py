...
class Model(nn.Module):
    def __init__(self, configs, num_stacks=1,
                 num_levels=2, positionalE=False, modified=True, RIN=False):
        super(Model, self).__init__()
        self.input_dim = configs.enc_in
        self.input_len = configs.seq_len
        self.pred_len = configs.pred_len
        self.hidden_size = configs.hidden_size #8inECL
        self.num_levels = num_levels
        self.groups = configs.enc_in
        ...
        self.pe = positionalE
        self.RIN = RIN
        self.num_decoder_layer = configs.d_layers # 2inECL
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        self.projection1 = nn.Conv1d(self.input_len, self.pred_len, kernel_size=1, stride=1, bias=False)
        ...
        if self.num_decoder_layer > 1:
            self.projection1 = nn.Linear(self.input_len, self.pred_len)
        ...
        if self.single_step_output_One:  # only output the N_th timestep.
            if self.stacks == 2:
                if self.concat_len:
                    ...
                    self.projection2 = nn.Conv1d(self.input_len + self.pred_len, 1,
                                                 kernel_size=1, bias=False)
            ...
            if self.stacks == 2:
                if self.concat_len:
                    ...
                    self.projection2 = nn.Conv1d(self.input_len + self.pred_len, self.pred_len,
                                                 kernel_size=1, bias=False)
